...
class Bittrex(Exchange):
    ...
    def market_sell(self, coin, base='BTC', quantity=0):
        pair = self.get_pair(coin, base)
        price = self.get_price(coin) * 0.95
        res = self.api.sell_limit(pair, quantity, price)['result']
        if not res:
            return None
        else:
            info = self.get_order(res['uuid'])['result']
            # if not info['QuantityRemaining'] == 0:
            #     print(info['QuantityRemaining'])
            #     assert(info['QuantityRemaining']) == 0
            pair = info['Exchange'].split('-')
            coin, base = pair[1], pair[0]
            return {
                'exchange': self.name,
                'side': 'sell',
                'pair': self.get_my_pair(coin, base),
                'price': info['PricePerUnit'],
                'quantity': info['Quantity'],
                'total': info['Price'],
                'fee': info['CommissionPaid'],
                'id': info['OrderUuid'],
                'id2': info['Sentinel']
            }
